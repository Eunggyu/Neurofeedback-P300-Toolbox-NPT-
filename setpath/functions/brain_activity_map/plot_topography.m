...
% ----------------------------------------------------------------------- %
function h = plot_topography(app, ch_list, values, make_contour, system, ...
    ...
    if ischar(system)
        switch system
            case '10-20_19ch'
                % 10-20 system (19channel)
                load('System_10-20_19ch.mat', 'locations');
            case '10-20_29ch'
                % 10-20 system (29channel)
                load('System_10-20_29ch.mat', 'locations')
            case '10-10_39ch'
                % 10-10 system (39channel)
                load('System_10-10_39ch.mat', 'locations');
            case '10-10_64ch'
                % 10-10 system (64channel)
                load('System_10-10_64ch.mat', 'locations');
            case '10-10_72ch'
                % 10-10 system (72channel)
                load('System_10-10_72ch.mat', 'locations');
            otherwise
                % Custom path
    ...
    if ~iscell(ch_list)
        ...
        for ch = 1:length(ch_list)
            if isempty(find(strcmp(locations.labels,ch_list{ch}), 1))
    ...
    % Interpolating input data
        ...
        pcolor(app.PlotTopographyUIAxes, interp_x, interp_y, interp_z);
        shading(app.PlotTopographyUIAxes, 'interp');
        hold(app.PlotTopographyUIAxes, 'on');
        
        % Contour
        if make_contour
            warning('off', 'MATLAB:contour:ConstantData')
            [~, hfigc] = contour(app.PlotTopographyUIAxes, interp_x, interp_y, interp_z); 
    ...
    plot(app.PlotTopographyUIAxes, head_x, head_y, 'Color', 'k', 'LineWidth',4);
    hold(app.PlotTopographyUIAxes, 'on');
    ...
    plot(app.PlotTopographyUIAxes, nose_x, nose_y, 'Color', 'k', 'LineWidth',4);
    hold(app.PlotTopographyUIAxes, 'on');
    ...
    plot(app.PlotTopographyUIAxes, ear_x_right+head_rho+offset, ear_y_right, 'Color', 'k', 'LineWidth',4); hold(app.PlotTopographyUIAxes, 'on');
    plot(app.PlotTopographyUIAxes, ear_x_left-head_rho-offset, ear_y_left, 'Color', 'k', 'LineWidth',4); hold(app.PlotTopographyUIAxes, 'on');

    % Plotting the electrodes
    % [ch_x, ch_y] = pol2cart((pi/180).*(locations.theta(idx)+90), locations.radius(idx));
    if plot_channels, he = scatter(app.PlotTopographyUIAxes, ch_x, ch_y, 60,'k', 'LineWidth',1.5); end
    if plot_clabels, text(app.PlotTopographyUIAxes, ch_x, ch_y, ch_list); end
    if strcmp(system,'yokogawa'), delete(he); plot(app.PlotTopographyUIAxes, ch_x, ch_y, '.k'); end
    ...
    app.PlotTopographyUIAxes.XLim = [-L, L];
    app.PlotTopographyUIAxes.YLim = [-L, L];
    % colorbar(app.PlotTopographyUIAxes);   % Feel free to modify caxis after calling the function
    axis(app.PlotTopographyUIAxes, 'square');
    axis(app.PlotTopographyUIAxes, 'off');
    hold(app.PlotTopographyUIAxes, 'off');
    gcf = app.PlotTopographyUIAxes;
    h = gcf;
end
