...
for iDc = 1:(length(dcArray) - 1)
    ...
    % Filter segment
    EEG.data(1:size(EEG.data,1), dcArray(iDc):dcArray(iDc + 1) - 1) = fir_filterdcpadded(b, 1, EEG.data(1:size(EEG.data,1), dcArray(iDc):dcArray(iDc + 1) - 1)', causal, usefftfilt)';
