function PhaseLockingObject = PhaseLockingValue(Signal,Fs,FrequencyBand,filterorder,windowleng,overlap,indexPhase)
% PhaseLockingValue - Phase Locking among channels for a given frequency
% range
%
% Usage:
%   PhaseLockingObject =
%   PhaseLockingValue(Signal,Fs,FrequencyBand,filterorder
...
%
% Outputs:
%   PhaseLockingObject - update the Phase Locking Value Biomarker
%
% Example:
%    PhaseLocking8_13Hz =
%    PhaseLockingValue(Signal,Fs,[8 13],[],[],[],[1 1])
%
%    phase lock computed with sliding window of 1s with 20% overlap
%    PhaseLocking8_13Hz =
%    PhaseLockingValue(Signal,Fs,[8 13],[],1,20,[1 1])
%
%
%% Input checks
narginchk(3,8)
...
disp('PhaseLockingObject = PhaseLockingValue(Signal,Fs,FrequencyBand)')
disp(' ')
...
PhaseLockingObject.PLV = triu(PhaseLockingObject.PLV);
PhaseLockingObject.PLV = PhaseLockingObject.PLV + PhaseLockingObject.PLV';
PhaseLockingObject.PLV(eye(size(PhaseLockingObject.PLV))~=0)=1;
