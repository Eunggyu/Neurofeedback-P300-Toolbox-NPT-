function [] = topoplot_connect(app, displayStruct, loc_file)
...
CIRCGRID   = 300;       % number of angles to use in drawing circles
...
HEADRINGWIDTH    = .009;% width of the cartoon head ring
...
%%%%%%%%%%%%%%%%%%%%%%% Draw blank head %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
cla(app.PlotConnectivityUIAxes)
hold(app.PlotConnectivityUIAxes, 'on')
set(app.PlotConnectivityUIAxes,'Xlim',[-rmax rmax]*AXHEADFAC,'Ylim',[-rmax rmax]*AXHEADFAC)
...
%
headx = [[rx(:)' rx(1) ]*(hin+hwidth)  [rx(:)' rx(1)]*hin]*1.2;
heady = [[ry(:)' ry(1) ]*(hin+hwidth)  [ry(:)' ry(1)]*hin]*1.2;

patch(app.PlotConnectivityUIAxes, headx,heady,ones(size(headx)),HEADCOLOR,'edgecolor',HEADCOLOR); hold(app.PlotConnectivityUIAxes, 'on');

%%%%%%%%%%%%%%%%%%% Plot ears and nose %%%%%%%%%%%%%%%%%%%%%%%%%%%
%
base  = rmax*1.2;
basex = 0.18*rmax*1.2;                   % nose width
tip   = 1.15*rmax*1.2;
tiphw = .04*rmax*1.2;                    % nose tip half width
tipr  = .01*rmax;                    % nose tip rounding
q = .04; % ear lengthening
EarX  = [.497-.005  .510  .518  .5299 .5419  .54    .547   .532   .510   .489-.005]*1.2; % rmax = 0.5
EarY  = [q+.0555 q+.0775 q+.0783 q+.0746 q+.0555 -.0055 -.0932 -.1313 -.1384 -.1199];
sf    = headrad/plotrad;

plot3(app.PlotConnectivityUIAxes, [basex;tiphw;0;-tiphw;-basex]*sf,[base;tip-tipr;tip;tip-tipr;base]*sf,...
    2*ones(size([basex;tiphw;0;-tiphw;-basex])),...
    'Color',HEADCOLOR,'LineWidth',HLINEWIDTH);                 % plot nose
plot3(app.PlotConnectivityUIAxes, EarX*sf,EarY*sf,2*ones(size(EarX)),'color',HEADCOLOR,'LineWidth',HLINEWIDTH)    % plot left ear
plot3(app.PlotConnectivityUIAxes, -EarX*sf,EarY*sf,2*ones(size(EarY)),'color',HEADCOLOR,'LineWidth',HLINEWIDTH)   % plot right ear

%
% %%%%%%%%%%%%%%%%%%% Show electrode information %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
gca = app.PlotConnectivityUIAxes;
plotax = gca;
axis(app.PlotConnectivityUIAxes, 'square');                                           % make plotax square
axis(app.PlotConnectivityUIAxes, 'off');
...
%         displayStruct.connectStrengthLimits = [min(displayStruct.connectStrength), max(displayStruct.connectStrength)];
        displayStruct.connectStrengthLimits = [0, 1];
    end
...
for kk = 1:numChanPairs
    plot3(app.PlotConnectivityUIAxes, y(displayStruct.chanPairs(kk, :)), x(displayStruct.chanPairs(kk, :)), [ELECTRODE_HEIGHT, ELECTRODE_HEIGHT], 'LineWidth', 2, 'Color', cM(cmapPos(kk), :));
end
% praneeth - ends
set(app.PlotConnectivityUIAxes, 'color', BACKCOLOR);
return;